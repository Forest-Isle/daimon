...
- 相关 Go 验证已通过。
- 新配置键已加入 `configs/daimon.example.yaml` 或明确说明为内部字段。
- 新工具声明了 capability、审批行为和并发安全性。
- 新 Gateway feature 已在 `internal/gateway/subsystem_feature.go` 注册，并在 `internal/feature` 定义。
