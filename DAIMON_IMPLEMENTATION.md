...
| Phase 3 sleep+proposals+shadow | 🟡 sleep 近完 (J1..J12) | digest/drift/synthesize/rollup/reconcile/proposals引擎/distill检测半+干净执行双代理(J11 tool-failure+J12 action-verified)+**distill-promote v1(候选→惰性SKILL.md草稿)** + replay 读侧/--against/回归集+金丝雀门 + **shadow 影子周报(§4.7)** 已落；余 distill 草稿→active转正(Canary→反射表)、proposals 投递 UX |
...
- **余（J10+，多数受阻于数据管线，已标前置）**：
  - **distill（§4.8 灵魂，最高带病转正风险）** — 检测半已落(J10)+干净执行双代理已落(J11 tool_failures + J12 unverified_actions)+**草稿生成 v1 已落(distill-promote, 候选→惰性 SKILL.md 草稿入 staging)**。promotion 半仍需：(1) **情节模式聚类**——journal outcome 仅 summary 文本，"重复≥3"需 LLM/嵌入聚类(verified 信号 J11+J12 已就位; 注: J10 检测的 LLM prompt 已做基础聚类)；(2) **草稿→active 转正**——把 staging 草稿经 J9 Canary 门 → first-exec-hold → attention 反射表，是 autonomous-执行写入，**CLAUDE.md 强制 Codex 反谄媚审查**。Canary 门（J9）已备, 草稿基底(distill-promote)已备。注: J12 的 verified 是"当前可 verified 的行动是否都 verified"；让 Compensable/Irreversible 也能客观 verified 的机制是后续切片(本切片如实反映现状，不放宽门)。
