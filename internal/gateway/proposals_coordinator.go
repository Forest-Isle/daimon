...
// Accept marks a live proposal accepted, then fires its action plan. The state
// transition gates first: Decide updates only a still-pending row, so a second
// accept (double-tap, redelivery race, concurrent callbacks) fails before any
// episode fires — accept is exactly-once. A proposal with no action plan is
// recorded as accepted but fires nothing (purely informational).
//
// Fire-failure window: if Decide succeeds but fire returns an error, the proposal
// is left accepted yet no episode ran, and the same id can no longer be retried
// through this path (state != pending). This is deliberately not an outbox — the
// loss self-heals: an accepted title no longer blocks PendingTitles, so while the
// underlying commitment is still due the next anticipation cycle re-proposes it.
// The caller logs the error so the failure is visible, not silent.
