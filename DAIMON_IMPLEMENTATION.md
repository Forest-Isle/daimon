...
- **余（J10+，多数受阻于数据管线，已标前置）**：
  ...
  - **subagent→episode（§4.3）** — **slice 1 已落(flag-gated kernel routing, 默认关; 子代理开 flag 走情节内核→强制 Outcome 交账+治理+成本归类, commit 54db5ce)**。余切：Outcome.Status→SubAgentResult.Status 传播、`Episode{ID,Parent}` 对象模型+显式父链、ExecModeBackground 路由、replay-bus 统一(子代理内部交换上全局 bus)、"10 并行情节无串扰"§4.3 验收。
