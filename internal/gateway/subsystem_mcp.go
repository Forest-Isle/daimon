...
func (ms *MCPSubsystem) Stop(_ context.Context) error {
	if ms.Manager != nil { return ms.Manager.Close() }
	return nil
}
...
func (ms *MCPSubsystem) WatchDir(ctx context.Context, cfg *config.Config) {
	...
	for {
		...
		case <-ticker.C:
			...
			if desired == nil { desired = make(map[string]config.MCPServerConfig) }
			for name, srv := range cfg.Tools.MCP.Servers { desired[name] = srv }
			if ms.Manager != nil { ms.Manager.SyncServers(ctx, desired, nil) }
		}
	}
}
