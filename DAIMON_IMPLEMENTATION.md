...
| Phase 4 economy+selfops+sensors | 🟡 economy 成本+ROI 报表完 (C1..C2d+J11+J12+C2e-1) | token 归因+记录基底(迁移034)+月报CLI+配置定价+activity-class+by-class$+ROI-by-class(clean 率 value 代理) 已落；余 C2e-2(throttle 控制流)、selfops、sensors 未开工 |
...
- **C2e-2（cost/ROI throttle，待实施，task #14）** — 控制流切片(Codex 审):超预算/负 ROI class 自动降级(路由更便宜模型/降频),**绝不 WakeUser**;基于 C2e-1 的 per-class clean-率+成本。须可观测+可逆,默认保守。有意不进 C2e-1 只读报表。
