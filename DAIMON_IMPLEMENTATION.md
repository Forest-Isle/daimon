...
- **conformance 复验 ✅（2026-06-15）** `make test`(fts5,CGO,race,30 pkg) 全绿、0 race、vet clean。J8/J9/CF6/CF7 因 relay 401 自审：核验 reconcile 的 delete+correction 原子性与 keep-guard、`fact.delete` 的 kind='fact' 守卫（test `TestListFactsFiltersKindAndFactDeleteGuardsAuditRows`）、episode 每条失败路径（stream/ApplyOutcome 错误）均经 `failEpisode` 留 journal 痕迹、status 枚举强制——确认守不变量 #1/#3/#4。已实施模块全部符合蓝图。
- **余（J10+，多数受阻于数据管线，已标前置）**：
  - **distill（§4.8 灵魂，最高带病转正风险）** — 需先补两条数据管线：(1) **每情节 verified 信号**——action `Receipt.Verified` 当前未按 episode 聚合，"全 verified"判据缺数据源（拟用 outcome `status=done ∧ ¬salvaged` 作保守代理，须标注为偏离蓝图严格语义）；(2) **情节模式聚类**——journal outcome 仅 summary 文本，"重复≥3"需 LLM/嵌入聚类。产物（skill 草稿 → Canary 门 → first-exec-hold → attention 反射表）是 autonomous-执行写入，**CLAUDE.md 强制 Codex 反谄媚审查**，relay 恢复前不落 promotion 半。Canary 门（J9）已备。
