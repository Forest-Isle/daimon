...
// Always saves to ~/.daimon/config.yaml (global user config).
func runSetupWizard() (string, error) {
	...
	savePath := filepath.Join(appdir.BaseDir(), "config.yaml")

	// ── Generate config ────────────────────────────────────────────
	configYAML := fmt.Sprintf(`# Daimon configuration — generated by setup wizard.
...
`,
	...
	doneContent :=
		lipgloss.NewStyle().Bold(true).Foreground(lipgloss.Color("#2EC49C")).Render("✓  Configuration complete!") +
			...
			lipgloss.NewStyle().Foreground(lipgloss.Color("#787882")).Render("Run: daimon tui")
