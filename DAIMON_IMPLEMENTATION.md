...
| Phase 3 sleep+proposals+shadow | 🟡 sleep 近完 (J1..J12) | digest/drift/synthesize/rollup/reconcile/proposals引擎/distill检测半+干净执行双代理(J11 tool-failure+J12 action-verified) + replay 读侧/--against/回归集+金丝雀门 已落；余 distill-promotion 半(聚类)、proposals 投递 UX、shadow(mind 拆出后)未开工 |
...
| **P3-I** 🟡 | mind 拆出 + 影子脑 | 6-10d | mind-split done(provider/cognition 契约拆入 `internal/mind`，3 阶段别名桥，agent→mind 单向零环，33 pkg 绿，commits 19a0055→9faeccf); 余: Shadow 影子脑(`Caps.CacheBreakpoints` + Cognize 订阅 dry-run→replay 对比) |
...
- **余 §4.7 Shadow 增量2-3 + thinking：** 增量2=mind.Shadow 类型+采样订阅（`heart_dispatch.go:68` Cognize→`RunInternalEpisode` 处复制给影子 runner）+ **episode 级交账重定向**（`RunInternalEpisode` 内 `world.ApplyOutcome` 写真 journal/world→影子必须不污染真传记；近不变量#3 交账强制，需谨慎：影子 outcome captured-not-persisted 或写影子 store）；增量3=影子周报（复用 J7 rescore Judge 出「每千 token 质量分」对比，**价值大半已被 `daimon replay --against <shadow-config>` 覆盖**=对最近录制情节离线对比）。thinking 通道跨 provider 统一：OpenAI provider 零 reasoning 处理，需选 reasoning_content/effort 约定，当前 Claude 路径为主→暂缓（非 speculative 原则）。
...
- **余（J10+，多数受阻于数据管线，已标前置）**：
  - **distill（§4.8 灵魂，最高带病转正风险）** — 检测半已落(J10)+干净执行双代理已落(J11 tool_failures + J12 unverified_actions)。promotion 半仍需：(1) **情节模式聚类**——journal outcome 仅 summary 文本，"重复≥3"需 LLM/嵌入聚类(verified 信号 J11+J12 已就位)；(2) 产物（skill 草稿 → Canary 门 → first-exec-hold → attention 反射表）是 autonomous-执行写入，**CLAUDE.md 强制 Codex 反谄媚审查**。Canary 门（J9）已备。注: J12 的 verified 是"当前可 verified 的行动是否都 verified"；让 Compensable/Irreversible 也能客观 verified 的机制是后续切片(本切片如实反映现状，不放宽门)。
