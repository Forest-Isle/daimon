...
func (SimpleLoop) Execute(ctx context.Context, a *Agent, ch channel.Channel, msg channel.InboundMessage, sess *session.Session) error {
	...
	for iteration := 0; iteration < maxIter; iteration++ {
		...
		if streamErr != nil {
			...
			return simpleNonStreaming(ctx, a, ch, sess, target, systemPrompt, maxIter)
		}
...
// computeBudgetPressure generates a warning string based on iteration and token pressure.
func computeBudgetPressure(iteration, maxIter int, sess *session.Session, systemPrompt string, ctxMgr ContextManager) string {
	var warnings []string
