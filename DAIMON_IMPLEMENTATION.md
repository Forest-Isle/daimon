...
| Phase 4 economy+selfops+sensors | 🟡 economy 成本报表完 (C1..C2d+J11+J12) | token 归因+记录基底(迁移034)+月报CLI+配置定价+activity-class+by-class$ 已落；J12 解开 C2e 所需的 per-episode action-verified 信号；余 C2e(ROI 分母+throttle)、selfops、sensors 未开工 |
...
- **C2e（待实施，task #12）** — ROI 分母（value-per-class：proposals adopted / verified actions / clean-vs-salvaged|tool-failure|unverified-action 比例）+ ROI=value/cost 报表 + 超预算/负 ROI class 自动降级 throttle（绝不 WakeUser）。**J12 已落 per-episode action-verified 信号**（`unverified_actions` in outcome detail）→ 解开此前的 action-level verified 真值阻塞；ROI 分母现可用 outcome detail 的 clean/tool_failures/unverified_actions 三态 + outcome status(done/blocked/handed_off) 作 value 代理。
